...
function addHeaderActionCallBackV4(entityType, appSchema, actionAnnotation, headerActions, oDataServiceAVT) {
    return (item, index) => {
        if ((!item['Determining'] && item.$Type === "com.sap.vocabularies.UI.v1.DataFieldForAction" /* UIAnnotationTypes.DataFieldForAction */) ||
            ...
            const actionDefinitionKey = `HeaderAction<${(0, common_1.prepareRef)(stableId)}>`;
...
function handleLineItem(facet, facetKey, appSchema, lineItemFacet, oDataServiceAVT) {
    ...
    (0, utils_1.addLineItemsTypeToSchema)(dataForLineItem, ux_specification_types_1.DefinitionName.ObjectPageTableColumn);
...
function handleFormIdentification(facet, facetKey, sections, appSchema, sectionPrefix) {
    ...
    (0, common_1.handleForm)(sectionPrefix, facet, formSection, appSchema, ux_specification_types_1.FioriElementsVersion.v4, facetKey);
}
/**
...
function addSections(generateParameters, appSchema, v4Page) {
    ...
    const facets = (0, common_1.getObjectPageFacets)(generateParameters.entitySet.entityType, generateParameters.serviceAVT, ux_specification_types_1.FioriElementsVersion.v4, generateParameters.logger);
    if (!facets) {
...
function addRelatedFacetKeysType(generateParameters, schema, entitySetName, pages) {
    Object.keys(pages).forEach((element) => {
        if (pages[element].options &&
            ...
            const facetSections = (0, common_1.getObjectPageFacetSection)(generateParameters.entitySet.entityType, generateParameters.serviceAVT, generateParameters.logger);
...
function addHeader(generateParameters, appSchema, v4Page) {
    ...
    // Add header sections
    if (entityType) {
        appSchema.definitions[ux_specification_types_1.DefinitionName.HeaderSections][ux_specification_types_1.SchemaTag.annotationPath] = `/${entityType.fullyQualifiedName}/@${"com.sap.vocabularies.UI.v1.HeaderFacets" /* UIAnnotationTerms.HeaderFacets */}`;
    }
 ...
 */
function generateObjectPageSchemaV4(generateParameters, genericSchema, entitySet) {
    ...
    if (entitySet) {
        ...
        const v4Page = (0, utils_1.findPageV4)(pages, ux_specification_types_1.v4.FE_TEMPLATE_V4_OBJECT_PAGE, entitySet, generateParameters.logger);
        if (!v4Page) {
            return appSchema;
        }
        // Add Header
        ...
        if (entitySet?.entityType) {
            sections[ux_specification_types_1.SchemaTag.annotationPath] = `/${entitySet.entityType.fullyQualifiedName}/@${"com.sap.vocabularies.UI.v1.Facets" /* UIAnnotationTerms.Facets */}`;
        }
        ...
        (0, common_1.addFooterActions)(appSchema, generateParameters);
    }
